module_name = 'ThisPico_F_v18.py'

import RemoteControl_v21 as RemoteControl
...
class ThisVSYS(GPIO.Volts):
    def __init__(self):
        super().__init__('VSYS',29, 3.0, 5.2)
        ThisPico.add(self)
    def close(self):
        ThisPico.remove(self)
        super().close()
...
class ThisThrottle(RemoteControl.Joystick):
    def __init__(self, interpolator):
        self.tsm = RemoteControl.StateMachine(name='Throttle SM', code='MEASURE', pin_no=3)
        super().__init__(name='Throttle', state_machine=self.tsm, interpolator=interpolator)
        ThisPico.add(self)
    def close(self):
        self.tsm.close()
        ThisPico.remove(self)
...
class ThisAileron(RemoteControl.Joystick):
    def __init__(self, interpolator):
        self.tsm = RemoteControl.StateMachine(name='Aileron SM', code='MEASURE', pin_no=4)
        super().__init__(name='Aileron', state_machine=self.tsm, interpolator=interpolator)
        ThisPico.add(self)
    def close(self):
        self.tsm.close()
        ThisPico.remove(self)
...
class ThisFlap(RemoteControl.Joystick):
    def __init__(self, interpolator):
        self.tsm = RemoteControl.StateMachine(name='Flap SM', code='MEASURE', pin_no=5)
        super().__init__(name='Flap', state_machine=self.tsm, interpolator=interpolator)
        ThisPico.add(self)
    def close(self):
        self.tsm.close()
        ThisPico.remove(self)
...
class ThisRemoteControlWithHeadlights(RemoteControl.RemoteControlWithHeadlights):
    def __init__(self, drive_train):
        ...
        int_s = RemoteControl.Interpolator('Steering Interpolator', [40, 56, 74, 76, 93, 110], [100.0, 100.0, 0.0, 0.0, -100.0, -100.0])
        int_k = RemoteControl.Interpolator('Knob Interpolator', [40, 50, 74, 76, 101, 110], [-100.0, -100.0, 0.0, 0.0, 100.0, 100.0])
        int_w = RemoteControl.Interpolator('Switch Interpolator', [0, 74, 76, 150], [-100.0, -100.0, 100.0, 100.0])
        left_up_down = ThisThrottle(int_t)
        left_sideways=None
        right_up_down=None
        right_sideways = ThisAileron(int_s)
        self.kick_switch = ThisFlap(None)
        mode_switch=None
        super().__init__(
                 'PicoF Remote',
                 left_side,
