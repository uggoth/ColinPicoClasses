...
class StateMachine(ColObjects.ColObj):
    ...
    def __init__(self, name, code, pin_no, hertz=100000):
        ...
        if code not in StateMachine.valid_codes:
            print ('****', code, 'not in', StateMachine.valid_codes)
            return None
        self.code = code
        if GPIOPico.GPIO.allocated[pin_no] != 'FREE':
            print ('****', pin_no, 'not free')
            return None
        self.pin_no = pin_no
        self.gpio = GPIOPico.ControlPin(self.name + '_control', self.pin_no)
        self.pin = self.gpio.pin
        found_free_state_machine = False
        for i in range(len(StateMachine.allocated)):
            if StateMachine.allocated[i] == 'FREE':
                if code == 'MEASURE':
                    self.instance = rp2.StateMachine(i, measure, freq=hertz, in_base=self.pin, jmp_pin=self.pin)
                    self.state_machine_no = i
                    StateMachine.allocated[i] = self
                    found_free_state_machine = True
                    break
                else:
                    print ('**** code', code,'not implemented yet')
                    return None
                    break
        if not found_free_state_machine:
            print ('**** no free state machine found')
            return None
        self.valid = True
    ...
    def close(self):
        if self.valid:
            ...
            StateMachine.allocated[self.state_machine_no] = 'FREE'
            self.gpio.close()
            super().close()
...
StateMachine.allocated[0] = GPIOPico.ControlPin('Reserved for neopixel',2)
